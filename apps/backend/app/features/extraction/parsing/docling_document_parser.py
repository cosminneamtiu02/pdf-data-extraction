...
PdfPreflight = Callable[[bytes], int]
...
`PdfPasswordProtectedError` for encrypted PDFs. On success, returns the PDF's
page count so the parser can enforce `max_pdf_pages` *before* triggering
Docling's full conversion pipeline (which includes OCR and is the expensive
cost the page-count cap is meant to defend against — PDFX-E003-F004
