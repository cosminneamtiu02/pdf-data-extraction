...
- **Monorepo tooling (preserved).** `pnpm 10` + Node 22 are retained for the existing workspace shell even though no frontend ships; the template's CI, Dependabot auto-merge, pre-commit, and Taskfile runner are preserved unchanged.
